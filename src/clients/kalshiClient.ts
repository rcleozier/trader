...
export class KalshiClient {
   ...
   * - cba, nbl, euro (alt basketball, spread-farming only, no external odds)
   */
  async fetchMarkets(
    seriesTicker: string,
    sport: 'nba' | 'nfl' | 'nhl' | 'ncaab' | 'ncaaf' | 'cba' | 'nbl' | 'euro' = 'nba'
  ): Promise<Market[]> {
    try {
      ...
      if (allMarkets.length === 0 && (sport === 'ncaaf' || sport === 'ncaab' || sport === 'cba' || sport === 'nbl' || sport === 'euro')) {
        for (const tickerToTry of uniqueSeriesTickers) {
          try {
            const marketsResponse = await this.marketsApi.getMarkets(
              1000, // limit
      ...
      const gameMarkets = allMarkets.filter((m: any) => {
        ...
        const isCollegeSport = sport === 'ncaaf' || sport === 'ncaab' || sport === 'cba' || sport === 'nbl' || sport === 'euro';
        ...
        // These tickers often have longer abbreviations (e.g., KXNCAAFGAME-..., KXNCAAMBGAME-..., KXCBAGAME-..., KXNBLGAME-..., KXEUROLEAGUEGAME-...)
  ...
  private extractGameInfo(
    ...
    const teamAbbrevMap =
      sport === 'nfl'
        ...
        : sport === 'nhl'
        ? nhlTeamAbbrevMap
        : sport === 'ncaab' || sport === 'ncaaf' || sport === 'cba' || sport === 'nbl' || sport === 'euro'
        ? {} // Empty map for college/CBA/NBL/Euroleague - will parse from ticker
        : nbaTeamAbbrevMap;
    const seriesPrefix =
      sport === 'nfl'
        ? 'KXNFLGAME'
